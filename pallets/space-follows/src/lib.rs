...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn follow_space(origin: OriginFor<T>, space_id: SpaceId) -> DispatchResultWithPostInfo {
        ...
        #[pallet::weight(10_000 + T::DbWeight::get().reads_writes(5, 5))]
        pub fn unfollow_space(origin: OriginFor<T>, space_id: SpaceId) -> DispatchResultWithPostInfo {
            let follower = ensure_signed(origin)?;
    ...
    pub type SpaceFollowers<T: Config> = StorageMap<_, Twox64Concat, SpaceId, Vec<T::AccountId>, ValueQuery>;

    #[pallet::storage]
    #[pallet::getter(fn space_followed_by_account)]
    pub type SpaceFollowedByAccount<T: Config> = StorageMap<_, Blake2_128Concat, (T::AccountId, SpaceId), bool, ValueQuery>;

    #[pallet::storage]
    #[pallet::getter(fn spaces_followed_by_account)]
    pub type SpacesFollowedByAccount<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, Vec<SpaceId>, ValueQuery>;


    impl<T: Config> Pallet<T> {
        fn add_space_follower(follower: T::AccountId, space: &mut Space<T>) -> DispatchResultWithPostInfo {
            space.inc_followers();
        ...
        pub fn unfollow_space_by_account(follower: T::AccountId, space_id: SpaceId) -> DispatchResultWithPostInfo {
            let space = &mut Spaces::require_space(space_id)?;
            space.dec_followers();
    ...
    impl<T: Config> BeforeSpaceCreated<T> for Pallet<T> {
        fn before_space_created(creator: T::AccountId, space: &mut Space<T>) -> DispatchResultWithPostInfo {
