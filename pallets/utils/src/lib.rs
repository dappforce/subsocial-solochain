...
pub mod pallet {
    use super::*;
    use frame_support::{migration, pallet_prelude::*};
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        fn on_runtime_upgrade() -> Weight {
            let old_pallet_prefix = "UtilsModule";
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn lowercase_and_validate_a_handle(handle: Vec<u8>) -> Result<Vec<u8>, DispatchError> {
            // Check if a handle length fits into min/max length constraints:
                ...
                handle.len() >= T::MinHandleLen::get() as usize,
                Error::<T>::HandleIsTooShort
            );
            ensure!(
                handle.len() <= T::MaxHandleLen::get() as usize,
                Error::<T>::HandleIsTooLong
            );

            let handle_in_lowercase = Self::lowercase_handle(handle);

            // Check if a handle contains only valid chars: 0-9, a-z, _.
            let is_only_valid_chars = handle_in_lowercase
                .iter()
                .all(|&x| Self::is_valid_handle_char(x));
...
pub fn log_2(x: u32) -> Option<u32> {
    if x > 0 {
        Some(num_bits::<u32>() as u32 - x.leading_zeros() - 1)
    } else {
        None
    }
}
