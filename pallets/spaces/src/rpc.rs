...
use pallet_utils::{
    bool_to_option,
    rpc::{FlatContent, FlatWhoAndWhen, ShouldSkip},
    SpaceId,
};
...
#[cfg(feature = "std")]
fn bytes_to_string<S>(field: &Option<Vec<u8>>, serializer: S) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
    let field_unwrapped = field.clone().unwrap_or_default();
    // If Bytes slice is invalid, then empty string will be returned
    serializer.serialize_str(std::str::from_utf8(&field_unwrapped).unwrap_or_default())
}
...
impl<T: Config> Pallet<T> {
    ...
    fn get_space_ids_by_owner<F: FnMut(&Space<T>) -> bool>(
        owner: T::AccountId,
        mut compare_fn: F,
    ) -> Vec<SpaceId> {
